...
function buildBeamSectorsUI() {
  ...
  root.innerHTML = SECTORS.map(
    (s) => `
    <div class="subhead">${s.label} 배근${s.key === "M" ? ' <button type="button" class="btn" style="padding:2px 8px;font-size:11px" data-copy-m="1">→ I/J에 복사</button>' : ""}</div>
    <div class="row2">
      <div class="field"><label for="BEAM-${s.key}-topName">상부 규격</label><input id="BEAM-${s.key}-topName" placeholder="D25"></div>
      <div class="field"><label for="BEAM-${s.key}-topNum">상부 개수</label><input id="BEAM-${s.key}-topNum" type="number"></div>
    </div>
    <div class="row2">
      <div class="field"><label for="BEAM-${s.key}-botName">하부 규격</label><input id="BEAM-${s.key}-botName" placeholder="D22"></div>
      <div class="field"><label for="BEAM-${s.key}-botNum">하부 개수</label><input id="BEAM-${s.key}-botNum" type="number"></div>
    </div>
    <div class="row3">
      <div class="field"><label for="BEAM-${s.key}-shearName">스터럽 규격</label><input id="BEAM-${s.key}-shearName" placeholder="D13"></div>
      <div class="field"><label for="BEAM-${s.key}-shearLeg">leg 수</label><input id="BEAM-${s.key}-shearLeg" type="number"></div>
      <div class="field"><label for="BEAM-${s.key}-shearDist">간격</label><input id="BEAM-${s.key}-shearDist" type="number" step="any"></div>
    </div>
    <div class="row2">
      <div class="field"><label for="BEAM-${s.key}-skinName">측면근 규격</label><input id="BEAM-${s.key}-skinName" placeholder="D13"></div>
      <div class="field"><label for="BEAM-${s.key}-skinNum">측면근 개수</label><input id="BEAM-${s.key}-skinNum" type="number"></div>
    </div>
  `
  ).join("");
...
function fillBeamForm(payload) {
  ...
  for (const s of SECTORS) {
    const sector = it[`BAR_SECTOR_${s.key}`] || {};
    const top = (sector.vMAIN_BAR_TOP || [])[0] || {};
    const bot = (sector.vMAIN_BAR_BOT || [])[0] || {};
    const shear = sector.SHEAR_BAR || {};
    setV(`BEAM-${s.key}-topName`, top.NAME);
    setV(`BEAM-${s.key}-topNum`, top.NUM);
    setV(`BEAM-${s.key}-botName`, bot.NAME);
    setV(`BEAM-${s.key}-botNum`, bot.NUM);
    setV(`BEAM-${s.key}-shearName`, shear.NAME);
    setV(`BEAM-${s.key}-shearLeg`, shear.LEG);
    setV(`BEAM-${s.key}-shearDist`, shear.DIST);
    setV(`BEAM-${s.key}-skinName`, sector.SKIN_BAR_NAME);
    setV(`BEAM-${s.key}-skinNum`, sector.SKIN_BAR_NUM);
  }
...
document.querySelectorAll('[data-action="list"]').forEach((btn) => {
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      sel.innerHTML =
        '<option value="">-- 선택 --</option>' + keys.map((k) => `<option value="${k}">${k}</option>`).join("");
      document.getElementById(`${type}-keylist`).textContent = keys.length
        ? `${keys.length}개 조회됨: ${keys.join(", ")}`
        : "등록된 항목이 없습니다 (신규 key를 직접 입력해서 저장할 수 있습니다).";
      showStatus(type, true, `목록 ${keys.length}건 불러옴`);
    } catch (e) {
      showStatus(type, false, "목록 조회 중 오류: " + e);
    } finally {
      btn.disabled = false;
    }
  });
...
document.querySelectorAll('[data-action="save"]').forEach((btn) => {
  btn.addEventListener("click", async () => {
    ...
    if (!key) {
      showStatus(type, false, "단면번호/ID(key)를 입력하세요.");
      return;
    }
    ...
    try {
      ...
      showStatus(type, true, "Gen NX 모델에 반영되었습니다.");
      state.loaded[type] = payload;
      renderDiagram(type, "before", payload);
    } catch (e) {
      showStatus(type, false, "저장 중 오류: " + e);
    } finally {
      btn.disabled = false;
    }
  });
...
function drawBeamSvg(payload, dims, aria) {
  ...
  const caption = `B${nf(B)}×H${nf(H)} · 스터럽 ${nf(shear.NAME)}@${nf(shear.DIST)} (중앙 단면 기준)`;
...
function drawColumnOrBraceSvg(payload, dims, aria, isBrace) {
  ...
  const kind = isBrace ? "가새" : "기둥";
  const caption = `${kind} B${nf(B)}×H${nf(H)} · 후프 ${nf(se.NAME)} ${nf(se.LEG_Y)}x${nf(se.LEG_Z)}@${nf(se.DIST)}`;
...
function drawWallSvg(payload, dims, aria) {
  ...
  let caption = `두께${nf(thickness)} · 수직 ${nf(vr.NAME)}@${nf(vr.DIST)} · 수평 ${nf(hr.NAME)}@${nf(hr.DIST)}`;
  if (length > dispLen) caption += " (실제 길이 일부만 표시)";
...
function drawSectionSvg(type, payload, dims, aria) {
  if (!payload) return '<div class="sec-empty">데이터 없음</div>';
...
function renderDiagram(type, which, payload) {
  ...
  const label = which === "before" ? "불러온 값" : "현재 입력값";
  el.innerHTML = drawSectionSvg(type, payload, currentDims(type), `${type} ${label} 단면`);
}
